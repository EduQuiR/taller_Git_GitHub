...
from logica import JuegoLogica

class JuegoAdivina:
    def __init__(self, master):
        self.master = master
        master.configure(bg="#1e3213")  
        master.title("Adivina el Número")

        self.nombre = ""
        self.juego = JuegoLogica()
    ...
    def verificar_intento(self):
        try:
            ...
            self.juego.intentos += 1

            if intento < self.juego.numero_secreto:
                messagebox.showinfo("Pista", "Demasiado bajo. Intenta otra vez.")
            elif intento > self.juego.numero_secreto:
                messagebox.showinfo("Pista", "Demasiado alto. Intenta otra vez.")
            else:
                messagebox.showinfo("¡Ganaste!", f"¡Felicidades {self.nombre}! Adivinaste el número en {self.juego.intentos} intentos.")
                self.master.destroy()
        except ValueError:
